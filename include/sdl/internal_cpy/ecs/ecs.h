...
namespace sdl{
  namespace ecs{
    ...
    template<class T>
    class IAttachable{
      static_assert(api::is_component<T>::value, "Not a component");
    public:
    ...
    public:
      ...
      // Get a component
      template<class T>
      T& get(api::UnaryPredicate<T> const& f = [](auto const&){ return true; });

      // Check if a component exists
      template<class T>
      bool contains(api::UnaryPredicate<T> const& f = [](auto const&){ return true; }) const;

      // Remove a component
      template<class T>
      void remove(api::UnaryPredicate<T> const& f = [](auto const&){ return true; });
    ...
    auto get_iterator(api::UnaryPredicate<T> const& f, Iterator begin, Iterator end){
    ...
    template<class T>
    T& BaseComponent::get(api::UnaryPredicate<T> const& f){
      static_assert(api::is_component<T>::value, "Not a component");
    ...
    template<class T>
    bool BaseComponent::contains(api::UnaryPredicate<T> const& f) const{
      static_assert(api::is_component<T>::value, "Not a component");
      /*const auto it = std::find_if(components_.begin(), components_.end(), [&](auto const& c){
      const auto ptr = dynamic_cast<T*>(c.get());
      return (ptr != nullptr && f(*ptr));
    ...
    template<class T>
    void BaseComponent::remove(api::UnaryPredicate<T> const& f){
      static_assert(api::is_component<T>::value, "Not a component");
